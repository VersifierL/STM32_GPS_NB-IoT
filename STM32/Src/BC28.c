...
{	
		...
		strncat((char *)data_temp, (const char *)data,12);
...
int BC28_ALIYUN_Init()
{
    ...
    printf("atstr = %s \r\n",atstr);
		...
		translate_status = 1; //连接成功标志位
		return err;// 后续优化使用，目前还未用到
}

void BC28_ALIYUN_Senddata1(char *temp_data, char *humi_data, char *voltage_data)
{	
		char buf5[]={0x1A,0x0D,0x0A};//数据头，必须有
    ...
    HAL_Delay(30);
    HAL_UART_Transmit(&huart3, (uint8_t *)buf5, strlen(buf5), 30); 
